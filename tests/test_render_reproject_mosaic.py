...
def test_render_year_rejects_reprojected_tile_without_alpha(tmp_path: Path, monkeypatch) -> None:
    """A tile under _reprojected/ that lacks the gdalwarp alpha band cannot have
    its padding masked; the render must fail loudly rather than silently emit a
    mostly-black year."""
    size = 100
    target_bbox = render.BBox(min_x=0.0, min_y=0.0, max_x=float(size), max_y=float(size))
    ...
    tile = repro / "noalpha__epsg2180_x.tif"
    arr = np.full((size, size, 3), 200, dtype=np.uint8)
    pyvips.Image.new_from_memory(arr.tobytes(), size, size, 3, "uchar").tiffsave(str(tile))

    geo = render.GeoRef(
        origin_x=0.0, origin_y=float(size), pixel_size_x=1.0, pixel_size_y=1.0,
        width=size, height=size,
    )
    monkeypatch.setattr(render, "_read_georef", lambda _p: geo)

    with pytest.raises(RuntimeError, match="no alpha"):
        render._render_year(
            year=2014,
            assets=[tile],
            out_path=tmp_path / "year_2014.tiff",
            target_bbox=target_bbox,
            target_width=size,
            target_height=size,
            target_srs="EPSG:2180",
