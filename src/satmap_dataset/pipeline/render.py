...
def _render_year(
    ...
    for asset in assets:
        ...
        alpha = None
        if is_reprojected and tile.bands >= 4:
    ...
    # Read back from the saved raster: the canvas pipeline references
    # sequential-access source tiles already consumed by tiffsave, so re-reading
    # it (avg) would fail; the file reflects the actual written pixels.
    return _mean_rgb_from_file(out_path)
